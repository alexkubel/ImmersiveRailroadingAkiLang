...
public class RailInfo {
	...
	public static class TagMapper implements cam72cam.mod.serialization.TagMapper<RailInfo> {
		...
		private static RailInfo legacy(TagCompound nbt) {
			...
			RailSettings settings = new RailSettings(gauge, "default", type, type, length, quarters / 4F * 90, 1, type == TrackItems.SLOPE ? TrackSmoothing.NEITHER : TrackSmoothing.BOTH , new EndPointData(0), new EndPointData(10), RollAndOffsetInfo.getDefault(), RollAndOffsetInfo.getDefault(), TrackDirection.NONE, railBed, cam72cam.mod.item.ItemStack.EMPTY, false, false, 1,  1,1,1);
